...
const FilterRoomOfRentalBookingModal = ({ onClose, onSave, isOpen }) => {
    ...
    //Start Date
    function convertToVietnamDate(dateStr) {
    ...
    function convertToFullISOString(dateStr) {
        ...
        // Tạo Date với giờ hiện tại (theo máy local)
        const localDate = new Date(`${year}-${month}-${day}T${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}:${now.getSeconds().toString().padStart(2, '0')}.${now.getMilliseconds().toString().padStart(3, '0')}`);

        // Cộng thêm 7 tiếng để chuyển sang giờ Việt Nam (UTC+7)
        const vietnamTime = new Date(localDate.getTime() + 7 * 60 * 60 * 1000);
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const data = {
                startDate: convertToVietnamDate(formData.startDate),
            ...
            // toast.error('Không thể tìm thấy booking của phòng thuê: ' + error.message);
        }
    };
...
export default FilterRoomOfRentalBookingModal;
