...
import { FaMoneyBillWave, FaCalendarCheck, FaCheck } from 'react-icons/fa';
...
const Dashboard = () => {
    ...
    const [loyalCustomersData, setLoyalCustomersData] = useState({
        labels: [],
        datasets: [{
    ...
    const fetchHomestayName = useCallback(async () => {
        try {
            const response = await homestayAPI.getHomestaysById(homestayId);
            if (response?.statusCode === 200) {
                setHomestayName(response.data?.name || '');
            }
        } catch (error) {
            console.error('Error fetching homestay name:', error);
        }
    }, [homestayId]);
    ...
    const fetchStatsData = useCallback(async () => {
        try {
            ...
            const [revenueResponse, bookingsResponse] = await Promise.all([
                dashboardAPI.getTotalBookingsAndAmountForHomeStayByHomestayID(homestayId),
                dashboardAPI.getAllGetStaticBookingsByHomestayID(homestayId)
            ]);

            if (revenueResponse?.statusCode === 200 && bookingsResponse?.statusCode === 200) {
                const newStatsCards = [
                    {
                        title: 'Tổng doanh thu',
                    ...
                    {
                        title: 'Tổng đơn hoàn thành',
                        value: bookingsResponse.data?.bookingsComplete || 0,
                        icon: <FaCheck className="w-6 h-6" />,
                        color: 'from-purple-400 to-purple-600'
                    },
                    {
                        title: 'Tổng đơn hoàn trả',
                        value: bookingsResponse.data?.bookingsReturnRefund || 0,
                        icon: <HiMiniReceiptRefund className="w-6 h-6" />,
                        color: 'from-orange-400 to-orange-600'
                    }
                ];
    ...
    const fetchWeeklyData = useCallback(async (weekData) => {
        ...
        try {
            ...
            const response = await dashboardAPI.getTotalBookingsTotalBookingsAmountForHomeStay(
                homestayId,
                weekData.startDate,
                weekData.endDate,
                "day"
    ...
    const fetchLoyalCustomersData = useCallback(async () => {
        try {
            setIsLoading(prev => ({ ...prev, loyalCustomers: true }));

            const response = await dashboardAPI.getTopLoyalCustomers(homestayId, "5");

            if (response?.statusCode === 200) {
                const formatData = {
                    labels: response.data?.map(item => item?.customerName) || [],
                    datasets: [{
                        label: 'Số lần đặt phòng',
                        data: response.data?.map(item => item?.totalBookings) || [],
                        backgroundColor: [
                            'rgba(34, 197, 94, 0.8)',
                            'rgba(59, 130, 246, 0.8)',
                            'rgba(168, 85, 247, 0.8)',
                ...
                setLoyalCustomersData(formatData);
            }
        } catch (error) {
            console.error('Error fetching loyal customers data:', error);
        } finally {
            setIsLoading(prev => ({ ...prev, loyalCustomers: false }));
        }
    }, [homestayId]);
    ...
    const fetchRoomTypeData = useCallback(async () => {
        try {
            setIsLoading(prev => ({ ...prev, roomTypes: true }));
            ...
            setIsLoading(prev => ({ ...prev, roomTypes: false }));
        }
    }, [homestayId]);
    ...
    useEffect(() => {
        Promise.all([
            fetchHomestayName(),
            fetchStatsData(),
            fetchLoyalCustomersData(),
            fetchServiceData(),
            fetchRoomTypeData()
        ]);
    }, [fetchHomestayName, fetchStatsData, fetchLoyalCustomersData, fetchServiceData, fetchRoomTypeData]);
                                ...
                                options={{
                                    ...
                                    plugins: {
                                        ...
                                        tooltip: {
                                            ...
                                            callbacks: {
                                                label: (context) => {
                                                    return `Số lần đặt: ${context.parsed.x} lần`;
                                                }
                                            }
                                        }
                                    },
