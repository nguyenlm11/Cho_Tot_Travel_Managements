...
const RoomListFilterByHomestay = () => {
  ...
  const statsData = useMemo(() => [
    { label: 'Tổng số đặt dịch vụ', value: rooms.length, color: 'bg-blue-500', icon: <FaTag className="w-6 h-6" /> },
    { label: 'Đang phục vụ', value: rooms.filter(b => b.status === ServiceBookingStatus.InProgress).length, color: 'bg-green-500', icon: <FaCheck className="w-6 h-6" /> },
    { label: 'Đã xác nhận', value: rooms.filter(b => b.status === ServiceBookingStatus.Confirmed).length, color: 'bg-indigo-500', icon: <FaCheck className="w-6 h-6" /> },
    { label: 'Chờ xác nhận', value: rooms.filter(b => b.status === ServiceBookingStatus.Pending).length, color: 'bg-yellow-500', icon: <FaCalendarAlt className="w-6 h-6" /> }
  ], [rooms]);
